...
class PlatinumRetentionPlanTests:
    ...
    def test_retain_snapshot(self):
        ...
        assert self.retention_plan.retain_snapshot(retention_limit_year) \
            == False
        assert self.retention_plan.retain_snapshot(
            retention_limit_year - timedelta(days=130)) == False
        assert (
            self.retention_plan.retain_snapshot(retention_limit_year.replace(
                month=12,
                day=31
                )) == True
            )
